...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ha_Noi").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://caochungdat.me/docs/other/thinh`)).data.url)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://api-trunghieu.hieutrung38.repl.co/api/anosdeptrai.php`)).data.data, {
                    responseType: 'stream'
                })).data
            };
