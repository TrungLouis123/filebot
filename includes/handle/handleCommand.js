	...
	return async function ({
		...
		if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
			if (!ADMINBOT.includes(senderID) && adminbot.adminOnly)
				return api.sendMessage('[𝐁𝐨𝐭 𝐌𝐝𝐨𝐧𝐠] THÔNG BÁO:\nChỉ ADMIN có thể sử dụng Bot.!!!', threadID, messageID);
		}
        if(!ADMINBOT.includes(senderID) && !event.isGroup) {
            return api.sendMessage("[𝐁𝐨𝐭 𝐌𝐝𝐨𝐧𝐠] THÔNG BÁO:\nBạn cần nhắn tin trao đổi với Admin để sử dụng Bot.", event.threadID, event.messageID)
        }
		...
		if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true)
			return api.sendMessage('[𝐁𝐨𝐭 𝐌𝐝𝐨𝐧𝐠] THÔNG BÁO:\nChỉ ADMIN có thể sử dụng Bot.!!!', event.threadID, event.messageID);
			...
			if (!ADMINBOT.includes(senderID.toString())) {
				if (userBanned.has(senderID)) {
					...
					return api.sendMessage(global.getText("handleCommand", "userBanned", reason, dateAdded), threadID, async (err, info) => {
						await new Promise(resolve => setTimeout(resolve, 99999 * 1000));
			...
			if (!ADMINBOT.includes(senderID)) {
				...
				if (banUsers.includes(command.config.name))
					return api.sendMessage(global.getText("handleCommand", "commandUserBanned", command.config.name), threadID, async (err, info) => {
						await new Promise(resolve => setTimeout(resolve, 999999 * 1000));
						return api.unsendMessage(info.messageID);
					}, messageID);
			}
		}
